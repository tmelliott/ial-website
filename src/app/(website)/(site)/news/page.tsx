import { getPayload } from "payload";
import config from "@payload-config";
import NewsCard from "../../components/NewsCard";
import cn from "../../utils/cn";

export default async function Page() {
  const payload = await getPayload({ config });

  const { docs: news } = await payload.find({
    collection: "news",
    pagination: false,
    sort: "-date",
  });

  return (
    <div className="">
      <header className="bg-linear-170 from-15% from-[var(--color-bg-gradient-start)] to-[125%] to-[var(--color-bg-gradient-end)]  p-4 text-white shadow-sm">
        <div className="max-w-6xl flex flex-col gap-8 mx-auto mt-8 lg:mt-48 lg:mb-24">
          <h1 className="text-5xl font-display pb-12">Latest news</h1>
        </div>
      </header>

      <section className="-mt-24 mb-24 flex flex-col gap-8 lg:grid lg:grid-cols-3 lg:gap-12 max-w-6xl mx-auto">
        {news.map((item, index) => (
          <div
            key={item.id}
            className={cn(index > 3 || index === 0 ? "col-span-full" : "")}
          >
            <NewsCard
              id={item.id}
              featured={index === 1}
              display={index > 3 ? "row" : "card"}
            />
          </div>
        ))}
      </section>

      <div className="bg-linear-355 from-15% from-[var(--color-bg-gradient-start)] to-[125%] to-[var(--color-bg-gradient-end)]  p-4 h-68"></div>
    </div>
  );
}
